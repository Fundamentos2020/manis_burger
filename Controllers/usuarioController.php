<?php
    ...
    try {
        ...
        {
            $SQL='SELECT * FROM `usuario` WHERE `E-mail`="'.$_GET['correo'].'"';
            ...
            if(count($usuarios)==0)
            {
                ...
                $contraseña= password_hash( $nuevoUsuario->getContraseña(),PASSWORD_DEFAULT);
                ...
                $resp = array();
                $resp['mensaje']="El correo ya esta registrado";
            ...
            if($_GET['tarea']==='consultaUsuario')
            {
                   ...
                   // echo($SQL);
                    ...
                    if(count($usuarios)>0)
                    {
                        ...
                        if(password_verify($_GET['pwd'],$usuario['contraseña']))
                        {
                            echo(json_encode($usuarios));
                        }
                        else{
                            $resp = array();
                            $resp['mensaje']="Correo o contraseña invalida. Si no esta registrado registrese";
                            echo(json_encode($resp));
                        }
                    }
                    else{
